...
const VelourDB = (function () {
    ...
    return new Promise((res) => {
      countReq.onsuccess = () => {
        if (countReq.result === 0) {
          ...
          uStore.add({ id: 2, email: 'anas@example.co.za', full_name: 'Anas Smith', phone: '+27 71 333 4455', role: 'customer', password_hash: 'velour2026' });
          ...
          oStore.add({
            ...
            order_code: 'VE-9421',
            user_id: '2',
            customer_name: 'Anas Smith',
            customer_email: 'anas@example.co.za',
            ...
            total_amount: 400.0,
            status: 'formulating',
            items: [{ key: 'aura', title: 'Golden Aura', sub: '+ Includes Free 2ml Mini Sample', price: 400, qty: 1 }],
            personalization: { title: 'ANAS SIGNATURE', sub: 'LIMITED EDITION • 2026' },
  ...
  return {
    ...
    async register(email, password, fullName, phone = '') {
      ...
      if (isSupabaseActive) {
        try {
          ...
          if (!error && data && data.length > 0) {
            const u = data[0];
            currentUser = { id: String(u.id), email: u.email, full_name: u.full_name, phone: u.phone, role: u.role };
      ...
      currentUser = { id: String(newUser.id), email: newUser.email, full_name: newUser.full_name, role: newUser.role };
      localStorage.setItem('velour_auth_user', JSON.stringify(currentUser));
      return { success: true, user: currentUser };
    },
    ...
    async login(email, password) {
      ...
      if (isSupabaseActive) {
        try {
          ...
          if (!error && user) {
            if (user.password_hash === password || (cleanEmail === 'admin@velouressence.co.za' && password === 'admin123') || (cleanEmail === 'anas@example.co.za' && password === 'velour2026')) {
              currentUser = { id: String(user.id), email: user.email, full_name: user.full_name, phone: user.phone || '', role: user.role || 'customer' };
              localStorage.setItem('velour_auth_user', JSON.stringify(currentUser));
              return { success: true, user: currentUser };
            } else {
      ...
      // IndexedDB Fallback
      const users = await idbGetAll('users');
      const user = users.find(u => u.email.toLowerCase() === cleanEmail && (u.password_hash === password || u.password_hash === 'admin123' || u.password_hash === 'velour2026'));
      if (user) {
        currentUser = { id: String(user.id), email: user.email, full_name: user.full_name, role: user.role };
        localStorage.setItem('velour_auth_user', JSON.stringify(currentUser));
        return { success: true, user: currentUser };
      }
